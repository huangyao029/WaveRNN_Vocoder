...
class UpsampleNetwork(nn.Module):
    ...
    def __init__(self, feat_dims, upsample_scale, compute_dims, res_blocks, res_out_dims, pad):
        super().__init__()
        total_scale = np.cumproduct(upsample_scale)[-1]
        self.indent = pad * total_scale
        self.resnet = MelResNet(res_blocks, feat_dims, compute_dims, res_out_dims, pad)
        self.resnet_stretch = Stretch2d(total_scale, 1)
        self.up_layers = nn.ModuleList()
        for scale in upsample_scale:
            k_size = (1, scale * 2 + 1)
            padding = (0, scale)
...
class WaveRNN(nn.Module):
    ...
    def __init__(self, rnn_dims, fc_dims, bits, pad, upsample_factors, feat_dims, compute_dims,
                 res_out_dims, res_blocks, hop_length, sample_rate, mode = 'MOL'):
        super().__init__()
        self.mode = mode
        self.pad = pad
        if self.mode == 'RAW':
            ...
            RuntimeError('Unknow model mode value - ', self.mode)
        ...
        self.upsample = UpsampleNetwork(feat_dims, upsample_factors, compute_dims, res_blocks,
                                        res_out_dims, pad)
